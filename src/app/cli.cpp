...
#include "../../include/utils/MediaHttpServer.h"
#include "CLI/CLI.hpp"
...
void run_scan(ScanOptions scan_options,
              bool file_result,
...
int parse_cli(int argc, char** argv, const std::string& default_root, AppConfig& config) {
    ...
    app.add_option("-p,--path", config.root_path, "Root directory to scan")
        ->capture_default_str();
    app.add_option("-i,--interval", config.interval_spec, "Scan interval (e.g. 10s, 5min, 2h, 1d)")
        ->capture_default_str();
    app.add_flag("-r,--recursive", config.recursive, "Recursive scan (default true)");
    app.add_flag("-s,--symlinks", config.follow_symlinks, "Follow symbolic links");
    app.add_flag("-f,--file", config.run_with_file_result, "Get result as file");
    app.add_option("-o,--output", config.output_dir, "Directory to save .media_files (defaults to --path)");
    app.add_option("--allow-ext", config.allowed_ext, "Allow only these extensions (e.g. .mp3 .wav)")
        ->expected(-1);
    app.add_option("--block-ext", config.blocked_ext, "Exclude these extensions (e.g. .tmp .txt)")
        ->expected(-1);
...
void run_http_mode(const ScanOptions& scan_opts, const AppConfig& config) {
    ...
    std::thread scan_thread([&]() {
        run_scan_server_result(scan_opts, store, config.interval_spec);
    });
