...
struct ScanOptions {
  ...
  public:
    ...
    class Builder {
        ...
        std::unordered_set<std::string> audio_ext_ = { ".mp3", ".wav", ".flac", ".aac", ".ogg", ".m4a" };

        std::unordered_set<std::string> video_ext_ = { ".mp4", ".mkv", ".avi", ".mov", ".mpg", ".mpeg", ".webm" };

        std::unordered_set<std::string> image_ext_ = { ".jpg", ".jpeg", ".png", ".bmp", ".gif", ".tiff", ".webp" };

      public:
        Builder(std::filesystem::path root_path = "./", bool follow_symlinks = false, bool recursive = true);

        Builder &select_ext(const std::vector<std::string> &audio_ext);
        Builder &remove_ext(const std::vector<std::string> &audio_ext);

        Builder &set_root_path(std::filesystem::path path) {
            root_path_ = std::move(path);
            return *this;
        }
        Builder &set_follow_symlinks(bool v) {
            follow_symlinks_ = v;
            return *this;
        }
        Builder &set_recursive(bool v) {
            recursive_ = v;
            return *this;
        }
